...
public class MwClntService {
    ...
    public List< LoanServingDTO > getAllActiveClint( String user ) {
        Query query = em.createNativeQuery(
                "select clnt.clnt_id,clnt.clnt_seq,clnt.frst_nm, clnt.last_nm,sum(app.aprvd_loan_amt) loan_amt, sum(rcv.pymt_amt) rcvd_amt,dr.dth_rpt_seq,dr.amt,brnch.brnch_seq,e.EXP_SEQ\r\n"
                        ...
                        + "join mw_ref_cd_val val on val.ref_cd_seq=clnt.CLNT_STS_KEY and val.crnt_rec_flg=1 and val.del_flg=0 and val.ref_cd ='0005'\r\n"
                        ...
                        + "left outer join Mw_Exp e on e.EXP_REF=dr.dth_rpt_seq and e.CRNT_REC_FLG=1\r\n" + "where clnt.crnt_rec_flg=1 \r\n"
                        + "group by clnt.clnt_id,clnt.clnt_seq,clnt.frst_nm, clnt.last_nm,dr.dth_rpt_seq,dr.amt,brnch.brnch_seq,e.EXP_SEQ" )
