...
public class Queries {
	...
	public static String complianceLoansQuery = "with qry as (\r\n" + 
			"select /*+ materialize */\r\n" + 
			"        ap.loan_app_seq,\r\n" + 
			...
			"        case when get_biz_tnr(ap.loan_app_seq) > 0 then get_app_acty(ap.loan_app_seq) when get_sch_tnr(ap.loan_app_seq)>0 then 'School' else null end biz_acty,\r\n" + 
			...
			"        inst_adv,\r\n" + 
			"        (select max(crdt_scr) from mw_loan_app_crdt_scr scr where scr.crnt_rec_flg=1 and scr.loan_app_seq=ap.loan_app_seq) crdt_scr,\r\n" + 
			...
			"        get_od_info(ap.loan_app_seq,sysdate,'i') od_inst,\r\n" + 
			"        chk_num        \r\n" + 
			"        from mw_loan_app ap\r\n" + 
			...
			"         left outer join (select loan_app_seq,LISTAGG(chq_num,',') within group (order by loan_app_seq) chk_num\r\n" + 
			"            from mw_pdc_hdr pdh\r\n" + 
			"            join mw_pdc_dtl pdt on pdt.pdc_hdr_seq=pdh.pdc_hdr_seq and pdt.crnt_rec_flg=1\r\n" + 
			"            where pdh.crnt_rec_flg=1\r\n" + 
			"            group by pdh.loan_app_seq) pdc on pdc.loan_app_seq=ap.loan_app_seq\r\n" + 
			"    where ap.crnt_rec_flg=1 and ap.loan_app_sts=703\r\n" + 
			")\r\n" + 
			"select * from qry";
	
	
	public static String complianceLandingQuery = "SELECT	 brnch.brnch_seq, brnch.brnch_cd,brnch.brnch_nm, trgt.trgt, (select count(1) from mw_adt_vst vst \r\n" + 
			"join mw_ref_cd_val val on val.ref_cd_seq=vst.VST_STS_KEY and val.ref_cd='01385' and val.crnt_rec_flg=1 \r\n" + 
			"where vst.brnch_seq=trgt.brnch_seq and vst.crnt_rec_flg=1) as completed_visits\r\n" + 
			"			 FROM\r\n" + 
			"			    mw_adt_trgt trgt\r\n" + 
			"			    JOIN mw_brnch brnch ON brnch.brnch_seq = trgt.brnch_seq\r\n" + 
			"			                         AND brnch.crnt_rec_flg = 1\r\n" + 
			"            where trgt.crnt_rec_flg=1";
